...
1. Configuration DNS du domaine personnalisé `arvest.app` dans OVH (section `Web Cloud`, menu `Zones DNS`) :
...
2. Configuration du site web dans github pages :
    ...
    - Dans la section [Setting > Pages](https://github.com/arvest-data-in-context/arvest-pages-website/settings/pages) associé au dépôt du site web d'Arvest :
        - Renseigner `arvest.app` dans le champ `Custom domain`
    - Attendre que le DNS checking automatique de github pages se fasse (jusqu'à 72h d'attente).
    - Deux cas de figures possibles arrivé à cette étape :
        ...
        - Cas négatif : `Custom domain` est invalidé en rouge, il est alors impossible de cocher cette case
            - Dans ce cas => lien "marche à suivre détaillée" donné à la section **Procedure** pour tenter de cerner le problème 
...
Si l'ensemble de la procédure a bien fonctionné, quelque soit l'url tapée dans votre navigateur préféré (p. ex. http://www.arvest.app, https://arvest.app, arvest.app), doit vous faire entrer sur le site web d'Arvest (https://arvest.app/fr) par une connexion sécurisée, marquée "Vérifié par : Let's Encrypt" au niveau du petit cadenas situé à gauche de l'URL dans la barre de recherche de votre navigateur.
